function nmfm_coefficient(jet, periodicsolution::PsolLPC, τs, ap)
  ...
  dφ(τ, θ) = d_interpolateT(τ, θ, q1, T, ts, ncol) + 0.5 * θ * dd_interpolateT(τ, θ, γ, T, ts, ncol)
...
function normal_form_coefficients!(jet, branch, τs, ap)
  for (i, p) in enumerate(branch.points)
    branch.points[i] = @set p.nmfm = nmfm_coefficient(jet, p, τs, ap)
  end
end
