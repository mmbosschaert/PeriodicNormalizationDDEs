...
function normalform(jet, hopf::GenHopf, τs)
  ...
  # Applying the bordering technique
  # n = length(q)
  # borderMatrix = [Δ(λ) p; q' 0.0]
  # qSolution = borderMatrix \ [zeros(n); 1.0]
  # q = qSolution[1:n]
  # pSolution = [zeros(1, n) 1.0] / borderMatrix
  # p = pSolution[1:n]
  ...
  # p = [1.376227019198842 - 0.402805063505650im; -0.390532272403555 - 1.207397183321654im]
  # q = [-7.179596962446969e-01 - 1.387778780781446e-17im; 1.032988488481162e-17 - 6.960846748551662e-01im]
