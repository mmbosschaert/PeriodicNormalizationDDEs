...
function vec(p::psol_fold,_)
    [vcat(p.profile...); vcat(p.eigenvector...); p.parameters; p.period; p.beta ...]
end

function vec_to_point(vec,p_prev::psol_fold,_)
    dims = length(p_prev.profile[1])
    ncol = p_prev.ncol
    ntst = convert(Int,(length(p_prev.profile)-1)/ncol)
    psol_fold([reshape(vec[1:dims*(ntst*ncol+1)],2,:)[:,j] for j in 1:ntst*ncol+1],
...
function SetupLPCBranch(jet,fold_guess,τs; parameterbounds=nothing, δ=.001, δmin=1e-06, δmax=0.01, MaxNumberofSteps = 250)
    ...
    df = (x,psol1) -> defsystem_fold_jac_β(jet,
                psol_fold([reshape(x[1:dims*(ntst*ncol+1)],2,:)[:,j] for j in 1:ntst*ncol+1],
    ...
    # convert fold_guess to psol_fold
    fold_guess = psol_fold(
        fold_guess.profile,
    ...
    lpc_branch = (points = psol_fold[], tangents = [], stepsizes = [], 
        f = f, df = df,
