...
function stability(jet::Jet, p, τs; neigs=13, maxit=100, σ=nothing)
...
function stability(jet::Jet, branch::Branch, τs; neigs=13, maxit=100)
  for (i, p) in enumerate(branch.points)
    branch.points[i] = stability(jet, p, τs; neigs=neigs, maxit=maxit)
  end
end

function stability(jet::Jet, points::Vector{Any}, τs; neigs=13, maxit=100)
  for (i, p) in enumerate(points)
      points[i] = stability(jet, p, τs; neigs=neigs, maxit=maxit)
  end
end
